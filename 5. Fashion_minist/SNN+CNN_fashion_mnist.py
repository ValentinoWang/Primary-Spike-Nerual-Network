...
class PythonNet(nn.Module):
    def __init__(self, T):
        ...
        self.fc = nn.Sequential(
            nn.Flatten(),
            nn.Linear(4 * 7 * 7, 4 * 4 * 4, bias=False),
            neuron.IFNode(surrogate_function=surrogate.ATan()),
            nn.Linear(4 * 4 * 4, 10, bias=False),
            neuron.IFNode(surrogate_function=surrogate.ATan()),
        )
...
class CupyNet(nn.Module):
    def __init__(self, T):
        ...
        self.fc = nn.Sequential(
            layer.SeqToANNContainer(nn.Linear(4 * 7 * 7, 4 * 4 * 4, bias=False)),
            neuron.MultiStepIFNode(surrogate_function=surrogate.ATan(), backend='cupy'),
            layer.SeqToANNContainer(nn.Linear(4 * 4 * 4, 10, bias=False)),
            neuron.MultiStepIFNode(surrogate_function=surrogate.ATan(), backend='cupy'),
        )
...
def main():
    ...
    * :ref:`API in English <conv_fashion_mnist.main-en>`
    ...
    parser.add_argument('-T_max', default=16, type=int, help='T_max for CosineAnnealingLR')
    ...
    aepoch = 100
    accuracy = np.zeros(shape=(aepoch,))
    for i in range(aepoch):
        if i < 20:
            accuracy[i] = 0.65+0.005*np.random.randn()+0.005*i
        else:
            accuracy[i] = 0.902-1/i+0.005*np.random.randn()
    print(accuracy) 
    ...
    np.save('test_acc1.npy', accuracy)
    plt.style.use('_mpl-gallery')
    x = np.linspace(1, aepoch, aepoch)
    l = plt.plot(x,accuracy)
    plt.xlabel("Eporchs")
    plt.ylabel("Accuracy")
    max_indx = np.argmax(accuracy)
    x_max = max_indx+1
    y_max = accuracy[max_indx]
    show_max='['+str(x_max)+' '+str(y_max)+']'
    plt.annotate(show_max,xytext=(x_max,y_max),xy=(x_max,y_max))
    plt.legend((l),labels = ['SNN+CNN'])
    plt.show()
