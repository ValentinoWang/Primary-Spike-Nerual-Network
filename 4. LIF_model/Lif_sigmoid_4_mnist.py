...
parser.add_argument('-alpha', '-alpha', default=4.0, type=int, help='训练epoch，例如“100”\n Training epoch, e.g., "100"')
def main():
    ...
    net = nn.Sequential(
        ...
        neuron.LIFNode(tau=tau,surrogate_function=surrogate.Sigmoid(alpha=4.0, spiking=True))
    )
    ...
    with torch.no_grad():
        ...
        np.save("v_t_sigmoid_4.npy",v_t_array)
        s_t_array = output_layer.s_seq.cpu().numpy().squeeze().T  # s_t_array[i][j]表示神经元i在j时刻释放的脉冲，为0或1
        np.save("s_t_sigmoid_4.npy",s_t_array)

    train_accs = np.array(train_accs)
    np.save('train_sigmoid_4.npy', train_accs)
    test_accs = np.array(test_accs)
    np.save('test_sigmoid_4.npy', test_accs)

    x = np.linspace(1, epoch+1, epoch+1)
    plt.style.use('_mpl-gallery')
    l = plt.plot(x,test_accs)
    plt.xlabel("Eporchs")
    plt.ylabel("Accuracy")
    max_indx = np.argmax(test_accs)
    x_max = max_indx+1
    y_max = test_accs[max_indx]
    show_max='['+str(x_max)+' '+str(y_max)+']'
    plt.annotate(show_max,xytext=(x_max,y_max),xy=(x_max,y_max))
    plt.legend((l),labels = ['Sigmoid(x), alpha = 1'])
