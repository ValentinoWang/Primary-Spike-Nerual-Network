...
def main():
    ...
    net = nn.Sequential(
        ...
        neuron.LIFNode(tau=tau,surrogate_function=surrogate.Sigmoid(alpha=10.0, spiking=True))
    )
    ...
    with torch.no_grad():
        ...
        np.save("v_t_sigmoid_10.npy",v_t_array)
        s_t_array = output_layer.s_seq.cpu().numpy().squeeze().T  # s_t_array[i][j]表示神经元i在j时刻释放的脉冲，为0或1
        np.save("s_t_sigmoid_10.npy",s_t_array)
# 
    train_accs = np.array(train_accs)
    np.save('train_sigmoid_10.npy', train_accs)
    test_accs = np.array(test_accs)
    np.save('test_sigmoid_10.npy', test_accs)

    x = np.linspace(1, epoch+1, epoch+1)
    plt.style.use('_mpl-gallery')
    l = plt.plot(x,test_accs, color='deeppink')
    plt.xlabel("Eporchs")
    plt.ylabel("Accuracy")
    ...
    plt.legend((l),labels = ['Sigmoid(x), alpha = 10'])
