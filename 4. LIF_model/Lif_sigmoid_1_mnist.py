...
def main():
    ...
    net = nn.Sequential(
        ...
        neuron.LIFNode(tau=tau,surrogate_function=surrogate.Sigmoid(alpha=1.0, spiking=True))
    )
    ...
    with torch.no_grad():
        ...
        np.save("v_t_sigmoid_1.npy",v_t_array)
        s_t_array = output_layer.s_seq.cpu().numpy().squeeze().T  # s_t_array[i][j]表示神经元i在j时刻释放的脉冲，为0或1
        np.save("s_t_sigmoid_1.npy",s_t_array)

    train_accs = np.array(train_accs)
    np.save('train_sigmoid_1.npy', train_accs)
    test_accs = np.array(test_accs)
    np.save('test_sigmoid_1.npy', test_accs)
